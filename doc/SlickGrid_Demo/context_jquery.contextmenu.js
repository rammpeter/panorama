...
jQuery.fn.contextPopup = function(menuData) {
  ...
  function createMenu() {
    ...
    if (menuData.title) {
      $('<li class=contextmenu_header></li>').text(menuData.title).appendTo(menu);
    }
    ...
    menu.find('.contextmenu_header').text(menuData.title);
    return menu;
  }
  ...
  this.bind('contextmenu', function(e) {
    ...
    var bg = $('<div></div>')
      .css({left:0, top:0, width:'100%', height:'100%', position:'fixed', zIndex:1000000})  // fixed statt absolute, da sonst bei Scroll nach unten menu stehenbleibt, Zusatz Ramm 14.09.11
      .appendTo(document.body)
    ...
    menu.find('a').click(function() {
      bg.remove();
      menu.remove();
      return false;  // wichtig, da sonst an Beginn der Seite gescrollt wird, Zusatz Ramm 14.09.11
    });
