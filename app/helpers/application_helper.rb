...
module ApplicationHelper
  ...
  def prepare_param(param_sym, **options)
    ...
    raise "prepare_param: Parameter '#{param_sym}' has value '#{retval.inspect}' which is not a String" unless retval.is_a?(String) || retval.nil?
  ...
  def get_recall_params_info_for_render_page_caption
    {
        ...
        :action =>     "copy_to_clipboard(`#{request.parameters.except(:update_area, :browser_tab_id).to_json}`);  show_popup_message('#{t(:addition_copy_recall_params_answer, :default=>"Current request parameters are copied to clipboard.\nUse menu \"Spec. additions / Execute with given parameters\" to paste this parameters")}');"
    }
  end
