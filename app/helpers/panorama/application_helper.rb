...
module Panorama::ApplicationHelper
  ...
  def get_client_info_store
    if !defined?($login_client_store) || $login_client_store.nil?
      $login_client_store = ActiveSupport::Cache::FileStore.new(Panorama::EngineConfig.config.client_info_filename)
      Rails.logger.info("Local directory for client-info store is #{Panorama::EngineConfig.config.client_info_filename}")
    end

    $login_client_store
  rescue Exception =>e
    raise "Exception '#{e.message}' while creating file store at '#{Panorama::EngineConfig.config.client_info_filename}'"
  end
  ...
  def write_to_client_info_store(key, value)
    ...
    begin
      ...
      Rails.logger.error("Exception '#{e.message}' raised while writing file store at '#{Panorama::EngineConfig.config.client_info_filename}'")
      raise "Exception '#{e.message}' while writing file store at '#{Panorama::EngineConfig.config.client_info_filename}'"
    end
  end
  ...
  def sql_select_all(sql, modifier=nil, query_name = 'sql_select_all')   # Parameter String mit SQL oder Array mit SQL und Bindevariablen
    ...
    #   end
    #   h.extend Panorama::SelectHashHelper    # erlaubt, dass Element per Methode statt als Hash-Element zugegriffen werden können
  ...
  def sql_select_iterator(sql, modifier=nil, query_name = 'sql_select_iterator')
    Panorama::ConnectionHolder.check_for_open_connection(self)                            # ensure opened Oracle-connection
    stmt, binds = sql_prepare_binds(sql)
    stmt.gsub!(/\n[ \n]*\n/, "\n")                                                  # Remove empty lines in SQL-text
    Panorama::SqlSelectIterator.new(stmt, binds, modifier, get_current_database[:query_timeout], query_name)      # kann per Aufruf von each die einzelnen Records liefern
  end
  ...
  def sql_select_first_row(sql, query_name = 'sql_select_first_row')
    ...
    result[0]     #.extend Panorama::SelectHashHelper      # Erweitern Hash um Methodenzugriff auf Elemente
  end
  ...
  def set_dummy_db_connection
    Panorama::ConnectionHolder.establish_connection(:adapter  => 'nulldb')
  end
  ...
  def render_internal(update_area, controller, partial, additional_javascript_string = nil)
    ...
    respond_to do |format|
      format.js {render :js => "$('##{update_area}').html('#{escape_javascript(render_to_string :partial=>"panorama/#{controller}/#{partial}")
