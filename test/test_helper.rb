...
class ActiveSupport::TestCase
  ...
  setup do
    ...
    if management_pack_license == :panorama_sampler && (!defined?(@@awr_ash_sampler_ensured) || !@@awr_ash_sampler_ensured)
      @@awr_ash_sampler_ensured = true                                      # ensure that AWR Sampler is only ensured once per test class run, because it is time consuming
      ...
      PanoramaSamplerStructureCheck.do_check(prepare_panorama_sampler_thread_db_config, :ASH)         # Ensure that structures are existing
    end
  end
  ...
  def initialize_min_max_snap_id_and_times(time_format = :minutes)
    ...
    all_awr_dbids.each do |awr_dbid|
      unless four_valid_snaps_found?(snaps)
        ...
        Raise "ActiveSupport::TestCase.initialize_min_max_snap_id_and_times: DBID is nil for awr_dbid = #{awr_dbid}" if awr_dbid['dbid'].nil?
