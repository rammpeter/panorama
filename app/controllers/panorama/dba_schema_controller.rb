...
class DbaSchemaController < ApplicationController
  ...
  def show_object_size
    @tablespaces = sql_select_all("\
      ...
      ORDER BY 1 ");
    @tablespaces.insert(0, {:name=>all_dropdown_selector_name}.extend(Panorama::SelectHashHelper))
    ...
    @schemas = sql_select_all("\
      ...
      FROM DBA_Users
      ORDER BY 1 ");
    @schemas.insert(0, {:name=>all_dropdown_selector_name}.extend(Panorama::SelectHashHelper))
...
WHERE RowNum < 100
  ...
  def list_audit_trail_grouping(grouping, where_string, where_values, update_area, top_x)
    ...
    @audits.each do |a|
      a.extend Panorama::SelectHashHelper
    end
  ...
  def list_dbms_metadata_get_ddl
    ...
    ddl = sql_select_one ["SELECT DBMS_METADATA.GET_DDL(object_type => 'TABLE', schema => ?, name => ?) FROM DUAL", @owner, @table_name]
    ...
    respond_to do |format|
      format.js {render :js => "$('##{params[:update_area]}').html('<div class=\"yellow-panel\"><h3>DDL for table #{@owner}.#{@table_name} generated by DBMS_METADATA.GET_DDL</h3>#{my_html_escape(ddl)}</div>');"}
    end
