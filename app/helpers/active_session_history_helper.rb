...
module ActiveSessionHistoryHelper
  ...
  def session_statistics_key_rules
    ...
    unless @session_statistics_key_rules_hash
      ...
      @session_statistics_key_rules_hash["SQL Exec-ID"] = {:sql => "s.SQL_Exec_ID",       :sql_alias => "sql_exec_id",        :Name => 'SQL Exec-ID',   :Title => 'SQL Execution ID', :info_sql  => "MIN(SQL_Exec_Start)", :info_caption => "Exec. start time"} if session[:database].version >= "11.2"
      @session_statistics_key_rules_hash["Operation"]   = {:sql => "RTRIM(s.SQL_Plan_Operation||' '||s.SQL_Plan_Options)", :sql_alias => "operation", :Name => 'Operation', :Title => 'Operation of explain plan line' } if session[:database].version >= "11.2"
      ...
      @session_statistics_key_rules_hash["Machine"]     = {:sql => "s.Machine",           :sql_alias => "machine",            :Name => 'Machine',       :Title      => "Client machine" } if session[:database].version >= "11.2"
    end
    @session_statistics_key_rules_hash
  end
  ...
  def groupfilter_value(key, value=nil)
    unless @groupfilter_values_hash
      ...
      @groupfilter_values_hash["Additional Filter"]     = {:sql => "UPPER(s.Session_ID||s.Module||s.Action||s.Program||s.Machine) LIKE UPPER('%'||?||'%')", :already_bound => true }  # Such-Filter
