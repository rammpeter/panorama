# encoding: utf-8
class NoaController < ApplicationController
  ...
  def list_object_increase_detail
    ...
    @incs = sql_select_all ["
        ...
        FROM   (SELECT /*+ PARALLEL(s,2) */
                ...
                FROM   dbadmin.OG_SEG_SPACE_IN_TBS s
    ...
    respond_to do |format|
      format.js {render :js => "$('##{params[:update_area]}').html('#{j render_to_string :partial=>"list_object_increase_detail" }');"}
    end
  end
