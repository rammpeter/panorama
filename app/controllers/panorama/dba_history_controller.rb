    ...
    @multiplans.each do |mp|
      ...
      if get_db_version >= "11.2"     # Ab 11.2 sind ASH-Records mit Verweis auf Zeile des Ausführungsplans versehen
        ...
        mp[:plans].each do |p|
          ...
          p.extend Panorama::TolerantSelectHashHelper   # Erlaibt Methoden-Aufrufe auf nicht deklarierte Element mit return nil
        end
...
FROM (
  ...
  def show_system_statistics_historic
    ...
    @statclasses.each do |s|
      s.extend Panorama::SelectHashHelper
    end
  ...
  def list_mutex_statistics_historic_samples
    ...
    respond_to do |format|
      format.js {render :js => "$('##{params[:update_area]}').html('#{j render_to_string :partial=>"panorama/dragnet/list_dragnet_sql_result" }');"}
    end
  end
