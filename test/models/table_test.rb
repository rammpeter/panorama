...
class TableTest < ActiveSupport::TestCase
  ...
  test "references_from" do
    ...
    PanoramaConnection.sql_execute("CREATE INDEX IX_T1 ON #{PanoramaConnection.username}.T1 (ID1, ID2, ID3)")
    Table.new(PanoramaConnection.username, 'T1').references_from.each do |ref|
      assert_nil ref.min_index_name, "There should not be a protecting index for foreign key #{ref.constraint_name}"
    end

    # Create an index that is protecting the foreign key because the first column is the FK column
    PanoramaConnection.sql_execute("CREATE INDEX IX_T1 ON #{PanoramaConnection.username}.T1 (ID2, ID1, ID3)")
    Table.new(PanoramaConnection.username, 'T1').references_from.each do |ref|
      assert_not_nil ref.min_index_name, "There should be a protecting index for foreign key #{ref.constraint_name}"
    end
