...
class AdditionController < ApplicationController
      ...
      FROM   (SELECT Instance_Number, Owner, Name, Partition_Name,
              ...
              FROM   (SELECT Instance_Number, Owner, Name, #{partition_expression} Partition_Name,
                      ...
                      FROM   (SELECT o.*,
                              ...
                              FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Cache_Objects o
  ...
  def list_db_cache_historic_timeline
    ...
    singles = sql_select_all ["\
      ...
      FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Cache_Objects c
      JOIN   (
  ...
  def list_blocking_locks_history_sum
    ...
    @locks= sql_select_all ["\
      ...
      FROM   (SELECT l.*,
              ...
              FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Blocking_Locks l
              WHERE  Snapshot_Timestamp BETWEEN TO_DATE(?, '#{sql_datetime_mask(@time_selection_start)}') AND TO_DATE(?, '#{sql_datetime_mask(@time_selection_end)}')
              #{@where_string}
             )
  ...
  def list_blocking_locks_objects_locked
    ...
    @locks = sql_select_all ["\
      SELECT *
      FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Blocking_Locks
      WHERE  Snapshot_Timestamp = TO_DATE(?, '#{sql_datetime_second_mask}')
      AND    Instance_Number = ?
      AND    SID = ?
      AND    Serial_No = ?",
  ...
  def list_object_increase_detail
    ...
    @incs = sql_select_all ["\
      WITH Snapshots AS (SELECT Owner, Segment_Name, Segment_Type, Gather_Date,
                         ...
                         FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Object_Sizes
  ...
  def list_object_increase_timeline
    ...
    sizes = sql_select_all ["
        ...
        FROM   #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Object_Sizes s
    ...
    output = gen_slickgrid(@sizes, column_options, {
        ...
        :max_height       => 450,
        :caption          => "Size evolution over time grouped by #{groupby} from #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Object_Sizes#{", Tablespace='#{@tablespace_name}'" if @tablespace_name}#{", Schema='#{@schema_name}'" if @schema_name}"
    })
  ...
  def list_object_increase_object_timeline
    ...
    output = gen_slickgrid(@sizes,
                           column_options,
                           {
                               ...
                               :caption         => "Size evolution of object #{owner}.#{name} recorded in #{ThreadLocalStorage.panorama_sampler_schema}.Panorama_Object_Sizes",
                               :max_height      => 450
                           }
    )
...
COUNT(DISTINCT NVL(#{column_name}, #{local_replace})) #{column_alias}_Cnt"
  ...
  def read_session_stats
    ...
    thread = Thread.new do
      ThreadLocalStorage.set_connection_info_for_request(get_current_database)
