...
Theoretical net size of uncompressed object without free space calculated by Avg_Row_Len*Num_Rows considering also block header, PCT_FREE, INI_TRANS.

May be inaccurate due to old or partial analyze with estimated average row length or object compression or function expressions in index.
For exact values click for calculation with DBMS_SPACE.SPACE_USAGE."
  ...
  def calc_free_space_mb_by_avg_row_len(avg_row_len, num_rows, pct_free, ini_trans, blocksize, size_mb, segment_type, leaf_blocks)
    if segment_type['TABLE']
      ...
      if segment_type['INDEX']
        calc_free_space_mb_index(avg_row_len, num_rows, pct_free, ini_trans, blocksize, leaf_blocks)
      else
        nil
      end
    end
  ...
  def calc_free_space_pct_by_avg_row_len(avg_row_len, num_rows, pct_free, ini_trans, blocksize, size_mb, segment_type, leaf_blocks)
    if segment_type['TABLE']
      ...
      if segment_type['INDEX']
        free_mb = calc_free_space_mb_index(avg_row_len, num_rows, pct_free, ini_trans, blocksize, leaf_blocks)
      else
        free_mb = nil
      end
    end
  ...
  def calc_compression_ratio(avg_row_len, num_rows, pct_free, ini_trans, blocksize, size_mb, segment_type, leaf_blocks)
    if segment_type['TABLE']
      calc_needed_space_mb_table(avg_row_len, num_rows, pct_free, ini_trans, blocksize) / size_mb
    else
      if segment_type['INDEX']
  ...
  def calc_mbytes_uncompressed(avg_row_len, num_rows, pct_free, ini_trans, blocksize, size_mb, segment_type, leaf_blocks)
    if segment_type['TABLE']
      calc_needed_space_mb_table(avg_row_len, num_rows, pct_free, ini_trans, blocksize)
    else
      if segment_type['INDEX']
        size_mb - calc_free_space_mb_index(avg_row_len, num_rows, pct_free, ini_trans, blocksize, leaf_blocks)
      end
    end
  rescue
    nil
  ...
  def calc_needed_space_mb_table(avg_row_len, num_rows, pct_free, ini_trans, blocksize)
    ...
    (needed_blocks * blocksize).to_f / (1024 * 1024)
  rescue
    nil
  end
  ...
  def calc_free_space_mb_index(
    ...
    data_size = ((block_size - block_header_size(init_trans)) * (1 - pct_free/100.0) - table_directory_entry_size).to_i

    # allocated size in leaf blocks by rows
    net_size = num_rows * (avg_row_len + (avg_row_len > 250 ? 3 : 1) + rowid_size)

    # remaining free space in currently allocated leaf blocks
    (data_size * leaf_blocks - net_size) / (1024.0 * 1024.0)
  rescue
    nil
  end
  ...
  def unified_audit_trail_view_name
    if get_db_version >= '18' && PanoramaConnection.is_cdb? && PanoramaConnection.con_id == 1
