...
=begin
  def gen_html_table(data, column_options, global_options={})

    raise "gen_html_table: Parameter-Type #{data.class.name} found for parameter data, but Array expected" unless data.class == Array
    raise "gen_html_table: Parameter-Type #{column_options.class.name} found for parameter column_options, but Array expected"  unless column_options.class == Array

    # Berechnung von Spaltensummen
    col_index = 0
    column_options.each do |col|
      col[:name]  = "col#{col_index}";
      col[:index] = col_index;
    ...
    output = ""       # Puffer mit gesamtem generierten html- und js-Code

    output << "<h3>#{global_options[:caption]}</h3>" if global_options[:caption]

    output << "<div style='#{global_options[:div_style]}'>" if global_options[:div_style]   # Tabelle in DIV verpacken, wenn expliziter style angegeben

    # Aufbau der Context-Menü-Informationen fuer Spalten

    global_options[:caption] = escape_js_chars(global_options[:caption])  # Sonderzeichen in caption escapen

    column_options.each do |col|

      col[:column_class] = "#{table_id}_#{col[:name]}"
      # Jeder Spalte ein spezifisches Context-Menu anhängen
      # Inhalte Context-Menü
      context_menu_entries = [
          {:label  => "Nach dieser Spalte sortieren",
           :icon   => "images/asc.gif",
           :action => "function(){jQuery('##{table_id}').find('th.#{col[:column_class]}').click(); }"  # Click auf Header der aktuellen spalte simulieren
          }
      ]
      if plotting && !col[:plot_master]  # Nur anzeigen wenn Diagramm möglich, aber nicht für plot_master-spalte
        context_menu_entries << {
            :label  => "Spalte im Diagramm ein-/ausblenden",
            :icon   => "images/application-monitor.png",
            :action => "function(){plot_table_diagram('#{table_id}', '#{plot_area_id}', '#{global_options[:caption]}','#{col[:column_class]}', '#{global_options[:multiple_y_axes]}', '#{global_options[:show_y_axes]}')}"
        }
      end

      context_menu_entries.concat(global_options[:context_menu_entries]) if global_options[:context_menu_entries] && global_options[:context_menu_entries].class == Array
      context_menu_entries << global_options[:context_menu_entries] if global_options[:context_menu_entries] && global_options[:context_menu_entries].class == Hash
      context_menu_entries <<  {
          :label  => "Export in csv-Datei",
          :icon   => "images/folder.png",
          :action => "function(){jQuery('##{table_id}').table2CSV(); }"  # Click auf Header der aktuellen spalte simulieren
      }

      output << "<script>
                   jQuery(function() {
                     jQuery('.#{col[:column_class]}').contextPopup({
                       title: 'Spalte #{col[:caption]}',
                       items: ["
      context_menu_entries.each do |e|        # Iteration ueber Memü-Einträge
        output << "{label:'#{e[:label]}', "
        output <<  "icon:'#{e[:icon]}', "  if e[:icon]
        output <<  "icon:'images/page_white_text.png', "  unless e[:icon]   # default-Icon wenn keines vorgegeben
        output <<  "action:#{e[:action]} "
        output << "}, "
      end
      output[output.length-1]=" "           # Letztes Komma der Liste entfernen
      output << "      ]
                     });
                   });
                 </script>"
    ...
    output << "<table id='#{table_id}' class='tablesorter' width='#{global_options[:width]}' style='font-size:small;'>\n"

    # Aufbau Header-Zeile
    output << "<thead><tr>\n"
    column_options.each do |col|
      output << "<th"
      output << " class='#{col[:column_class]} #{col[:header_class]}"   # Verbindung ueber spaltenspez. Klassennamen mit Contextmenü
      output << " plot_master" if col[:plot_master]          # Spalte markiert als Master füe X-Achse des Diagrammes
      output << "'"                                          # Schlusstag für Class
      if col[:title]
        inhalt = eval_with_rec("\"#{col[:title]}\"", nil)        # Ersetzungen im string a'la "#{}" ermoeglichen
        while inhalt["'"]
          inhalt["'"] = "&#39;"     # einfache Hochkommas im Text fuer html escapen
        end
        output << " title='#{inhalt}'"
      end
    ...
    data.each do |rec|
      ...
      column_options.each do |col|
        output << "<td "
        output << " class='#{col[:column_class]}'"  # Verbindung ueber spaltenspez. Klassennamen mit Contextmenü
        inhalt = ""                                 # Initialisierung, falls weder data_title noch title belegt sind
        if col[:data_title]
          begin
            inhalt = col[:data_title].call(rec).to_s if col[:data_title].class == Proc # Ersetzungen im string a'la "#{}" ermoeglichen
            inhalt = eval_with_rec("\"#{col[:data_title]}\"", rec)  unless col[:data_title].class == Proc # Ersetzungen im string a'la "#{}" ermoeglichen
          rescue Exception => e
          ...
          if col[:title]
            inhalt = eval_with_rec("\"#{col[:title]}\"", rec)        # Ersetzungen im string a'la "#{}" ermoeglichen
            while inhalt["'"]
              inhalt["'"] = "&#39;"     # einfache Hochkommas im Text fuer html escapen
            end
          end
        end
        inhalt["%t"] = col[:title] if inhalt["%t"] && col[:title]   # einbetten title in :data_title, wenn per %t angewiesen

        if col[:sum] && col[:sum] != 0    # Ausgabe %-Anteil und spaltensumme im title
          recval = col[:show_pct_hint].call(rec)
          inhalt << " #{formattedNumber((recval ? recval : 0) * 100.to_f / col[:sum], 2) } % of column sum: #{formattedNumber(col[:sum])}"
        end
        output << " title='#{inhalt}'"

        if col[:data_style]
          output << " style='"
          output << eval_with_rec("\"#{col[:data_style]}\"", rec)   # Ersetzungen im string "#{}" ermoeglichen
          output << "'"
        else
          output << " style='#{col[:style]}'" if col[:style]
        end

        output << " align='#{col[:align]}'" if col[:align]
        output << ">"
        begin
          if col[:data].class == Proc                           # Variante Proc direkt Ausführung
            data = col[:data].call(rec)
            output << data.to_s if data
          end
          output << eval_with_rec("#{col[:data]}.to_s", rec) unless col[:data].class == Proc    # Variante String per eval ausführen
        rescue Exception => e
          raise "#{e.class.name}: #{e.message} Error processing :data-rule for column #{col[:caption]}"
        end
        output << "</td>"
      end
    ...
    output << "</table>"
    output << "</div>" if global_options[:div_style]   # Tabelle in DIV verpacken, wenn expliziter style angegeben
    output << "<div id='#{plot_area_id}'></div>" unless global_options[:plot_area]   # Zeichenflaeche als div anlegen, wenn noch nicht existiert

    output << "<script>"
    output << convert_column_options_to_js(table_id, column_options) # Javascript-Objekt mit Spaltenstruktur als data an Table
    output << "</script>"
...
=end
  ...
  # Ausgabe von JS für Einbettung in HTML zur Deklaration der JS-Funktion locale_translate
  def set_js_locale_translate
