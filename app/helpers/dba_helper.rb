...
module DbaHelper
  ...
  def object_nach_wait_parameter(instance, event, p1, p1raw, p1text, p2, p2raw, p2text, p3, p3raw, p3text)
    wordsize = get_wordsize    # Wortbreite in Byte
    case
      ...
      when p1text == "idn" && p2text == "value" && p3text == "where"
        if event.match('cursor: ') then
          ...
          "Blocking-SID=#{p2.to_i/2**(4*get_wordsize) }, Cursor-Hash-Value=#{p1.to_i} SQL-ID='#{cursor_rec.sql_id if cursor_rec}', User=#{cursor_rec.parsing_schema_name if cursor_rec}, #{cursor_rec.sql_text if cursor_rec}"
        else   # Mutex etc.
