...
#      Rails.logger.info "######################### #{start_index} #{sql[start_index, sql.length-start_index]}"
      get_table_and_view_names.each do |table|                                  # Check if table might be replaced by Panorama-Sampler
        if table[:table_name].upcase == up_sql[start_index, table[:table_name].length].gsub(/DBA_HIST/, 'PANORAMA')
          sql   .insert(start_index, "#{ThreadLocalStorage.panorama_sampler_schema}.")  # Add schema name before table_name
          up_sql.insert(start_index, "#{ThreadLocalStorage.panorama_sampler_schema}.")  # Increase size synchronously with sql
          start_index += ThreadLocalStorage.panorama_sampler_schema.length+1            # Increase Pointer by schemaname
